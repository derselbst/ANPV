...
struct DocumentView::Impl
{
    ...
    void onSetBackgroundColor()
    {
        ...
        QObject::connect(&colDiag, &QColorDialog::currentColorChanged, q, [&](const QColor & col)
        {
            this->scene->setBackgroundBrush(QBrush(col));
        });
    ...
    void createActions()
    {
        ...
        connect(this->actionShowScrollBars, &QAction::toggled, q, [&](bool checked)
        {
            ANPV::globalInstance()->setViewFlag(ViewFlag::ShowScrollBars, checked);
        });
